...
char** processoLista(char* str, int* size) {
...
void preencherCatalogo() {
    ...
    while (fgets(linha, MAX_LINE, file)) {
        ...
        novoShow->show_id = campoProcesso(campos[0]);
        novoShow->type = campoProcesso(campos[1]);
        novoShow->title = campoProcesso(campos[2]);
        novoShow->director = campoProcesso(campos[3]);
        novoShow->cast = processoLista(campos[4], &novoShow->castSize);
        novoShow->country = campoProcesso(campos[5]);

        // Trata campo de data
        char* dateStr = campoProcesso(campos[6]);
        ...
        novoShow->rating = campoProcesso(campos[8]);
        novoShow->duration = campoProcesso(campos[9]);
        novoShow->listed_in = processoLista(campos[10], &novoShow->listedInSize);
        novoShow->description = campoProcesso(campos[11]);
...
// Imprimir pilha do topo para o fundo - decrescente
...
int main() {
    ...
    Pilha pilha;
    inicializarPilha(&pilha, MAX_LISTA);
